...
class RegAgentGreedyDecode(GreedyDecodeBase):
    """
        input   20 map agent ego, 80 map ego
        output  80 agent
    """
    ...
    def _save_predicted_data(feature, output, curr_idx):
        ...
        直接保存最后一帧就可以
        """
        feature['agent']['position'][:, 1:, curr_idx] = output['agent']['position'][:, -1, 1:]    # [bs, A, dim]
        feature['agent']['heading'][:, 1:, curr_idx] = normalize_angle(output['agent']['heading'][:, -1, 1:, 0])
        feature['agent']['valid_mask'][:, 1:, curr_idx] = torch.gt(output['agent']['valid_mask'][:, -1, 1:, 0], 0)
    ...
    def _create_save_data(
        ...
        # 将ego赋值为GT
        predicted_data['agent']['position'][:, 0, init_T:] = data['agent']['position'][:, 0, init_T:]
        predicted_data['agent']['heading'][:, 0, init_T:] = data['agent']['heading'][:, 0, init_T:]
        predicted_data['agent']['valid_mask'][:, 0, init_T:] = data['agent']['valid_mask'][:, 0, init_T:]
