...
class CustomDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        # Fractions，取train val test数据的百分之几
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "fit":
            # Training Dataset
            train_samples = self._splitter.get_train_samples(   # Extracts a list of samples to be used for training. devkit源码，不改
                self._all_samples, self._worker
            )
            assert len(train_samples) > 0, "Splitter returned no training samples"

            self._train_set = create_dataset(   # 这个函数就对数据随机采样了一下，直接修改创建：ScenarioDataset
                train_samples,  # 各个父级目录
                self._feature_preprocessor, # 提取文件
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        if self._scenario_type_sampling_weights.enable:     # trian的时候没有给不同场景添加权重
