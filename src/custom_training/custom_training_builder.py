...
def build_lightning_module(
    ...
    if "custom_trainer" in cfg:
        if cfg.py_func=='test':     # 直接初始化test pl.module
            model = instantiate(
                cfg.custom_trainer,
                model=torch_module_wrapper,
                greedy_data_processor=instantiate(cfg.greedy_data_processor),
                greedy_decode=instantiate(cfg.greedy_decode)
            )
        else:
            model = instantiate(
                cfg.custom_trainer,
    ...
    return cast(pl.LightningModule, model)  # 将 model 转换为 pl.LightningModule 类型
...
def build_custom_trainer(cfg: DictConfig) -> pl.Trainer:
    ...
    # callbacks = [
    #     ModelCheckpoint(    # 配置ckpt保存位置，保存top几的模型
    #         dirpath=os.path.join(os.getcwd(), "checkpoints"),
    #         filename="epoch={epoch}-step={step}-val_loss={loss/val_loss:.4f}",
    #         auto_insert_metric_name=False,  # Is useful to set it to False when metric names contain / as this will result in extra folders
    ...
    #         save_last=True,
    #     ),
    #     RichModelSummary(max_depth=1),  # 模型摘要，深度1
    #     RichProgressBar(),  # 使用RichProgressBar进度条
    #     LearningRateMonitor(logging_interval="epoch"),  # 用于监视和记录学习率的变化
    # ]
    ...
    trainer = pl.Trainer(
        ...
        strategy=pl.strategies.DDPStrategy(timeout=timedelta(seconds=14400), find_unused_parameters=False), 
        **params,
    )
...
def build_training_engine(cfg: DictConfig, worker: WorkerPool) -> TrainingEngine:
    ...
    # 配置pl.trainer，比如ckpt保存位置，保存top几，日志打印之类
    trainer = build_custom_trainer(cfg)

    # Create model，用cfg实例化model：PlanningModel，不用看
    ...
    # Build lightning module，将 model 转换为 pl.LightningModule 类型
