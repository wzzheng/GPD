...
from src.utils.sledge_utils.pdm_enums import SE2Index
from src.utils.sledge_utils.pdm_geometry_utils import calculate_progress, normalize_angle
from src.utils.sledge_utils.pdm_array_representation import (
    array_to_states_se2,
    states_se2_to_array,
)
...
class PDMPath:
    ...
    def __init__(self, discrete_path: List[StateSE2]):
        ...
        self._discrete_path = discrete_path     # List[StateSE2]
        self._states_se2_array = states_se2_to_array(discrete_path)     # np.array [n, 3]
        self._states_se2_array[:, SE2Index.HEADING] = np.unwrap(self._states_se2_array[:, SE2Index.HEADING], axis=0)    # 将角度从-pi, pi转为-pi到正无穷，取消周期性
        self._progress = calculate_progress(discrete_path)  # 路径的每个点之间的距离，0-0的距离，0-1的距离，0-2的距离，以此类推

        self._linestring = linestrings(self._states_se2_array[:, : SE2Index.HEADING])   
        self._interpolator = interp1d(self._progress, self._states_se2_array, axis=0) # 距离插值器
