...
class SledgeRawFeatureBuilder(AbstractFeatureBuilder):
    ...
    def _compute_features(
        ...
        drivable_area_map = get_drivable_area_map(map_api, ego_state, self._config.radius)  # 所有可以开的polygon [polygon.id, polygon.point]
        ...
        static_objects = compute_static_object_features(    # 类似提取agent
            ego_state,
            detections,
            self._config.radius,
            # drivable_area_map,
        ...
        ego = compute_ego_features(ego_state)   # 他只记录最后一帧的ego，不记录全时间段的
