...
def get_lane_graph(
    ...
    for lane_id, lane in lane_dict.items():
        G.add_node(lane_id)     # 每个polygon都是一个node
...
def get_edge_graph(ego_state: EgoState, map_api: AbstractMap, radius: float) -> List[PDMPath]:
    ...
    G_lane, lane_dict = get_lane_graph(ego_state, map_api, radius)  # 创建一个图，连接连续的路
    ...
    for lane_id, lane in lane_dict.items():
        is_predecessor_stop = len(set(G_lane.predecessors(lane_id)) & set(stop_dict.keys())) > 0    # 检查其前驱车道是否在终止车道中
        if is_predecessor_stop:
            start_dict[lane_id] = lane

        is_successors_start = len(set(G_lane.successors(lane_id)) & set(start_dict.keys())) > 0     # 后继车道是否在起始车道中
    ...
    for start_lane_id, start_lane in start_dict.items():
        path_to_targets = find_path_to_targets(G_lane, start_lane_id, list(stop_dict.keys()))   # 查其实节点到多个目标节点的最短路径
        if path_to_targets:
            discrete_path = get_discrete_path(lane_dict, path_to_targets)   # 获取多个polygon(三条路，中心路的)，拼接起来的所有点
            local_discrete_path: npt.NDArray[np.float32] = convert_absolute_to_relative_poses(  # 转化到第20帧为中心的坐标系下
...
def compute_path_features(interpolatable_paths: List[PDMPath], pose_interval: float) -> SledgeVectorElement:
    ...
    if len(line_poses_list) > 0:    # padding
...
def compute_line_features(
    ...
    :return: raw sledge vector element for feature caching.
    """
    interpolatable_paths = get_edge_graph(ego_state, map_api, radius)   # 获得连续的路径
    return compute_path_features(interpolatable_paths, pose_interval)   # 按照给定的采样间隔生成路径上的位置点
