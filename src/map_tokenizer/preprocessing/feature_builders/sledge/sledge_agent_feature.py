    ...
    if len(agents_states_all) > 0:
        if drivable_area_map is not None:
            in_drivable_area = drivable_area_map.points_in_polygons(agents_states_all[..., AgentIndex.POINT]).any(
                axis=0      # BUG ？ 这一行很诡异，是将polygon看作一个封闭集合体，判断agent是不是在polygon内
            )
            agents_states_all = agents_states_all[in_drivable_area]     # 对于 [1, 1, 1, 0, 1, 1, 1, 0, 1, 0]的情况，直接把agent中间的waypoint给扣掉了，很神奇
