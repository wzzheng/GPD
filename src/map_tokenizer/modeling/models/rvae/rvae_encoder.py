...
class RVAEEncoder(nn.Module):
    ...
    def __init__(self, config: Union[RVAEConfig, VAEConfig]):
        ...
        # self.map_info_indices = [   # BUG 这里对于cos -180或180,这个地方有点，也会被抹除
        #     SledgeRasterIndex._LINE_X
        # ]
        self.map_info_indices = [
            SledgeRasterIndex._LINE_X,
            SledgeRasterIndex._LINE_Y
        ]
    ...
    def forward(self, raster: torch.Tensor) -> Latent:
        ...
        # raster = torch.where(raster > 0, self.ones_tensor, raster)
        raster = (raster.sum(1, keepdim=True) > 0).to(torch.float32)    # 修复bug
