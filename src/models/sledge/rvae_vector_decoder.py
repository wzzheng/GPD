...
from .utils.rvae_position_encoding import build_position_encoding
from .utils.rvae_transformer import Transformer
...
class RVAEVectorDecoder(nn.Module):
    ...
    def __init__(self, config: RVAEConfig):
        ...
        super(RVAEVectorDecoder, self).__init__()
        ...
        if config.split_latent:
            self._type_encoding = nn.Embedding(2, config.d_model)   # 让pos_emb可学习，同时2，是为了区分static和dynamic obj
    ...
    def from_config(
        ...
        **kwargs,
        ...
        return RVAEVectorDecoder(config=config_dataclass)
    ...
    def forward(self, latent: torch.Tensor) -> SledgeVector:
        ...
        if self._config.split_latent:   # 这部分，就是利用static的信息，做一个位置编码，加到dynamic信息上，我们不用dynamic信息，直接位置编码就可以
            static_latent, dynamic_latent = torch.chunk(latent, 2, dim=1)

            static_patches, dynamic_patches = (     # 这里patch_size=1，没什么效果
            ...
            num_patches = self._config.num_patches
            decoder_mask = None     # 这个decoder_mask，是为了让DETR时，static_query去查static，不要看agent；agent_query也不要看static，所以我们不需要
            static_latent = latent  # latent仅包含static信息
            patches = patchify(static_latent, self._config.patch_size)
            patches = patches.flatten(-2).permute(2, 0, 1)  # (p*p, b, d_patches)
            projected_patches = self._patch_projection(patches)  # (p*p, b, d_model)，预测出的图块[h*w, bs, dim]

            # 定义位置编码
            pos_embed = self._position_encoding(static_latent)  # (b, d_model, p, p)，这里就是一个单纯的sincos的pos_emb，和输入x没关系
            pos_embed = pos_embed.flatten(-2).permute(2, 0, 1)  # (p*p, b, d_model)

        query_embed = self._query_embedding.weight[:, None].repeat(1, b, 1) # 可学习变量

        hs_line = self._transformer(
        ...
        _ = torch.full((b, 2, 2), -10.0, device=device)    # 避免修改大量代码，注入一个占位tensor
        return SledgeVector(
            line_element,
