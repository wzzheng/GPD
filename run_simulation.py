...
def run_simulation(
    ...
    common_builder = set_up_common_builder(cfg=cfg, profiler_name=profiler_name)    # 一些基础的设置，不用看
    ...
    # Remove planner from config to make sure run_simulation does not receive multiple planner specifications.  planner=None直接跳过
    ...
    # Construct simulations.  planner=None直接跳过
    if isinstance(planners, AbstractPlanner):
        planners = [planners]
    runners = build_simulations(        # 构建每一个场景目录的planner模型
        cfg=cfg,
        callbacks=callbacks,
...
def instants_planner(config):
    imitation_planner = cast(AbstractPlanner, instantiate(config.planner))['imitation_planner']

    # load planner ckpt
    planner_ckpt = torch.load(config.planning_model_ckeckpoint)['state_dict']
    planner_adapt_ckpt = {}
    for k, v in planner_ckpt.items():
        planner_adapt_ckpt[k.replace('model.', '')] = v
    imitation_planner._planner.load_state_dict(planner_adapt_ckpt)

    raster_encode_ckpt = torch.load(config.map_raster_encode_ckeckpoint)
    raster_encode_adapt_ckpt = {}
    for k, v in raster_encode_ckpt.items():
        layer_name = k.replace('model.', '')
        if '_vector_quantization' in layer_name:
            layer_name = layer_name.replace('_vector_quantization.', '')
        raster_encode_adapt_ckpt[layer_name] = v
    imitation_planner._map_encoder.load_state_dict(raster_encode_adapt_ckpt)
    return imitation_planner
...
def main(cfg: DictConfig) -> None:
    ...
    # 提前加载planner
    imitation_planner = instants_planner(cfg)

    run_simulation(cfg=cfg, planners=imitation_planner)
